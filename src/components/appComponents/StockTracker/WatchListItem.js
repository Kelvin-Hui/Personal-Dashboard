...
export default function WatchListItem({ ticker, listName }) {
  ...
  function quotePrice(ticker) {
    //const d = new Date("Fri, 15 Jan 2021 08:30:00 GMT-0500");
    const d = new Date();
    ...
    >
      ...
      {quoteSummary.prepostprice ? (
        <ListItemText
          ...
          primary={
            <Typography style={styles.prepostdisplay}>
              ...
              <Typography
                style={{
                  color:
                    quoteSummary.prepostpercent.raw > 0 ? "#24A84A" : "red",
                  marginLeft: "5%",
                }}
              >
          ...
          secondary={<Typography>At Close: {quoteSummary.price}</Typography>}
        />
      ) : (
        <ListItemText
          style={styles.price_percent}
          primary={<Typography>{quoteSummary.price}</Typography>}
          secondary={<Typography>{quoteSummary.percent.fmt}</Typography>}
        />
      )}
      ...
      <IconButton
        ...
        onClick={() =>
          dispatch(deleteWatchList({ listName: listName, ticker: ticker }))
        }
      >
